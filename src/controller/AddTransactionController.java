        ...
        if(type.equals("debit") )
        {
            ...
            if(BankConnectionValidator.withdrawalAmountValid(accountBalance, amount))
            {
                addTransactionQuery = "INSERT INTO " + "transactions" +
                        ...
                        " values (" + amount + ", " + "CURDATE()" + ", '" + type + "', '" +
                        description + "', " + (accountBalance - amount) + ", " + clientAccountNumber + ", " + social +  ")";
                updateCheckingQuery = "INSERT INTO " + "transactions" +
                        " (amount, trans_date, trans_type, description, balance, chk_account_number, client_social)" +
                        " values (" + amount + ", " + "CURDATE()" + ", '" + type + "', '" +
                        description + "', " + (accountBalance + amount) + ", " + Integer.parseInt(description) + ", " + social +  ")";

                ajoutQuery = "UPDATE checking_account set balance = " + (accountBalance - amount) + " where account_number = " + clientAccountNumber;
                deleQuery = "UPDATE checking_account set balance = balance +" +  amount + " where account_number = " + Integer.parseInt(description);
                bankConnection.executeStatement(addTransactionQuery);
                bankConnection.executeStatement(updateCheckingQuery);
                bankConnection.executeStatement(ajoutQuery);
                bankConnection.executeStatement(deleQuery);
            }
        }
        else
        {
